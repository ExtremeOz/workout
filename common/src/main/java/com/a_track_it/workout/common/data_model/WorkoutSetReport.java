...
public class WorkoutSetReport {
    ...
    public void addWorkoutSetData(WorkoutSet workoutSet) {
        ...
        if(map.get(workoutSet.activityID) == null) {
            ...
            if (workoutSet.stepCount != null) w.stepCount += workoutSet.stepCount;
            w.duration += workoutSet.duration;
            if (workoutSet.rest_duration != null) w.rest_duration += workoutSet.rest_duration;
            w.pause_duration += workoutSet.pause_duration;
            if (workoutSet.wattsTotal != null) w.wattsTotal += workoutSet.wattsTotal;
            if (workoutSet.weightTotal != null) w.weightTotal += workoutSet.weightTotal;
            w.setCount += workoutSet.setCount;
            if (workoutSet.repCount != null) w.repCount += workoutSet.repCount;
        }
    }
    ...
    public long getTotalRestDuration() {
        ...
        for (Object key1 : keys) {
            ...
            if ((workoutSet.activityID != Constants.WORKOUT_TYPE_TIME) && (workoutSet.activityID != Constants.WORKOUT_TYPE_STILL)
                    && (workoutSet.activityID != Constants.WORKOUT_TYPE_INVEHICLE)) {
                totalDuration += (workoutSet.rest_duration != null) ?  workoutSet.rest_duration : 0;
            }
        }
        return totalDuration;
    }
    ...
    public void setStepData(WorkoutSet workoutSet) {
        if(map.get(workoutSet.activityID) == null) {
            ...
            WorkoutSet w = map.get(workoutSet.activityID);
            w.start = 0L; // TODO: Remove this when we have step summary cached
...
/*    public String toString() {
        ...
        for (Object key1 : keys) {
            ...
            result += Utilities.getFitnessActivityTextById( value.regionID) + " steps: " + value.stepCount + Constants.LINE_DELIMITER;
            result += Utilities.getWorkOutTextById(value.regionID) + " duration: " + getDurationBreakdown(value.duration) + Constants.LINE_DELIMITER;
        }
        return result;
    }*/
