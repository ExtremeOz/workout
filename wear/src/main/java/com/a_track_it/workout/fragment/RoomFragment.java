...
import com.a_track_it.workout.adapter.RecyclerViewAdapter;
...
import com.a_track_it.workout.common.UnitLocale;
import com.a_track_it.workout.common.Utilities;
import com.a_track_it.workout.common.VectorDrawableUtils;
...
import com.a_track_it.workout.common.data_model.ResistanceType;
...
import com.a_track_it.workout.common.user_model.LiveDataTimerViewModel;
...
import static com.a_track_it.workout.common.Constants.ATRACKIT_DRAWABLE;
...
import static com.a_track_it.workout.common.Constants.REPS_TAIL;
import static com.a_track_it.workout.common.Constants.SINGLE_INT;
...
import static com.a_track_it.workout.common.Constants.WORKOUT_SETUP;
...
//    private boolean DoBurnInProtection;
    ...
    private String sBPM_fit;
    private String sBPM_device;
    private String sBPM_device2;
    private String sStep_fit;
    private String sStep_device;
    private String sStep_device2;
    private String sLocation_fit;
    private String sMotion_fit;
    private String sBarometer_device;
    private String sStatus_device;
    private String sBattery_device;
    private String sDistance_fit;
    private String sAltitude_fit;
    private String sSpeed_distance;
    private String sTemperature_device;
    private String sHumidity_device;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        try {
            ...
            mReceiver = new CacheResultReceiver(mHandler);
        }catch(Exception e){
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        WorkoutViewModelFactory factory = com.a_track_it.workout.common.InjectorUtils.getWorkoutViewModelFactory(context);
        ...
        mWorkoutAdapter = new WorkoutAdapter(context,null,null, true);
        mWorkoutAdapter.setListType(false);

        List<Workout> items = new ArrayList<>();
        adapter = new RecyclerViewAdapter(items,context, ATRACKIT_EMPTY, false);
        mRecyclerView.setAdapter(adapter); //mWorkoutAdapter
        ...
        sBPM_fit = getString(R.string.sensor_fit_bpm);
        sBPM_device = getString(R.string.sensor_device_bpm);
        sBPM_device2 = getString(R.string.sensor_device2_bpm);
        sStep_fit = getString(R.string.sensor_fit_steps);
        sStep_device = getString(R.string.sensor_device_steps);
        sStep_device2 = getString(R.string.sensor_device2_steps);
        sLocation_fit = getString(R.string.nav_location).toLowerCase();
        sMotion_fit = getString(R.string.nav_motion).toLowerCase();
        sBarometer_device = getString(R.string.sensor_device_pressure);
        sStatus_device = getString(R.string.nav_status).toLowerCase();
        sBattery_device = getString(R.string.sensor_device_battery);
        sDistance_fit = getString(R.string.sensor_fit_distance);
        sAltitude_fit = getString(R.string.sensor_fit_altitude);
        sSpeed_distance = getString(R.string.sensor_device_speed);
        sTemperature_device = getString(R.string.sensor_device_temperature);
        sHumidity_device = getString(R.string.sensor_device_humidity);
        ...
        textViewCenter2.setOnClickListener(v -> { doSetViewCenter2();});
        ...
        final Observer<Integer> stateObserver = state->{
            ...
            switch (state) {
                ...
                case WORKOUT_PENDING:
                    ...
                    if (adapter.getItemCount() == 0)  //mWorkoutAdapter
                        mRecyclerView.setVisibility(RecyclerView.GONE);
                    else
        ...
        final Observer<String> userIDObserver = s -> {
            if ((s != null) && (s.length() > 0)){
                ...
                Log.w(LOG_TAG,"userIDObserver " + s);
                if (userPrefs != null){
        ...
        final CacheResultReceiver.Receiver resultReceiverSummary = new CacheResultReceiver.Receiver() {
            @Override
            public void onReceiveResult(int resultCode, Bundle resultData) {
                if (resultCode == 200) {
                    Log.e(LOG_TAG, "CacheResultReceiver.Receiver loaded ");
                    ...
                    if ((workoutList == null) || (workoutList.size() == 0))
                        ...
                        Runnable aRunnable = () -> {
                            Log.e(LOG_TAG, "receiver loading " + workoutList.size());
                            adapter.setItems(workoutList, pageTitle);
                            mRecyclerView.setVisibility(View.VISIBLE);
                        };
                        if (mHandler != null) mHandler.post(aRunnable);
                    }
                }
            }
        };
        mReceiver.setReceiver(resultReceiverSummary);
/*        mSavedStateViewModel.getToDoSets().observe(getViewLifecycleOwner(), workoutSets -> {
        ...
        mSavedStateViewModel.getLocationMsg().observe(getViewLifecycleOwner(), s -> {
            ...
            if ((iTag < DetectedActivity.WALKING) && (textViewCenter2 != null)) {
    ...
    private void doSetViewCenter2(){
        int currentState = mSavedStateViewModel.getState();
        boolean bUseLoc = (mMessagesViewModel.getUseLocation().getValue() != null) 
                ? mMessagesViewModel.getUseLocation().getValue() : false;
        ...
        if ((currentState != WORKOUT_PAUSED) && (currentState != WORKOUT_LIVE)) {
            ...
            if (iType == 2) {
                ...
                if (context != null) {
                    switch (currentState) {
                        case WORKOUT_SETUP:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_setup);
                            break;
                        case WORKOUT_TEMPLATE:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_template);
                            break;
                        case WORKOUT_INVALID:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_invalid);
                            break;
                        case WORKOUT_PENDING:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_pending);
                            break;
                        case WORKOUT_LIVE:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_live);
                            break;
                        case WORKOUT_COMPLETED:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_completed);
                            break;
                        case WORKOUT_PAUSED:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_paused);
                            break;
                        case WORKOUT_CALL_TO_LINE:
                            sValue = context.getString(com.a_track_it.workout.common.R.string.state_call);
                            break;
                    }
        ...
        mSavedStateViewModel.setViewState(9,iType);
        final String sMsg = sValue;
        final int foreColor = (context != null) ? ContextCompat.getColor(context, (!isAmbient) ?  R.color.primaryTextColor : R.color.ambientForeground):0;
        final Drawable finalDrawable = drawable;
        new Handler(Looper.getMainLooper()).post(() -> {
            if (finalDrawable != null) textViewCenter2.setCompoundDrawablesWithIntrinsicBounds(finalDrawable,null,null,null);
            textViewCenter2.setCompoundDrawablePadding(6);
            if ((sMsg != null) && (sMsg.length() > 0)) textViewCenter2.setText(sMsg);
            if (foreColor > 0) textViewCenter2.setTextColor(foreColor);
        });        
    }
    ...
    public void refreshSummaryData(){
        if (appPrefs.getLastUserID().length() > 0) {
            ...
            long endTime = System.currentTimeMillis();
            Log.e(LOG_TAG,"getReport " + sDeviceID);
            CacheManager.getReport(1, 0, mReceiver, getContext(),sUserID,sDeviceID, startTime, endTime);
        }
    }
            ...
            switch (state) {
                ...
                case WORKOUT_INVALID:
                    ...
                    try {
                        ...
                        mImageView.setBackgroundColor(bgColor1);
                        if (!isAmbient) circularRevealFromMiddle(revealCardView,2500);
                    }
                    break;
            }
        });
